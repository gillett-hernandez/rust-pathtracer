...
impl PDF for Lambertian {
    ...
    fn generate(&self, hit: &HitRecord, s: &Box<dyn Sampler>, wi: Vec3) -> Option<Vec3> {
        ...
        Some(
            frame
                .to_world(&random_cosine_direction(s.draw_2d()))
                .normalized(),
        )
    }
}
...
impl BRDF for Lambertian {
    fn f(&self, hit: &HitRecord, wi: Vec3, wo: Vec3) -> RGBColor {
        ...
        // assert!(wi * hit.normal > 0.0, "aligned is what we want, and wi (which is the negation of the incoming rays direction) and hit.normal were not aligned. {:?}.{:?}={}", wi, hit.normal, wi * hit.normal);
